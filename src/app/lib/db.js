...
export async function fetchSponsors() {
  const { data, error } = await supabase.from('sponsors').select('*').order('tier', { ascending: true }).order('sort_order', { ascending: true });
  if (error) throw error;
  return data || [];
}

export async function upsertSponsor(sponsor) {
  const { data, error } = await supabase.from('sponsors').upsert(sponsor).select().single();
  if (error) throw error;
  return data;
}
...
export async function uploadSponsorLogo(file) {
  ...
  if (error) {
    // Fall back to a compressed inline image if the bucket is missing
...
export function subscribeToSponsors(callback) {
  return supabase
    .channel('sponsors-changes')
    .on('postgres_changes', { event: '*', schema: 'public', table: 'sponsors' }, callback)
    .subscribe();
}
