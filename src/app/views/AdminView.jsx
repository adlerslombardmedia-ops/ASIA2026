...
export default function AdminView({ data, navigate }) {
      ...
      <div className="animate-fade" style={{ padding: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '80dvh' }}>
        <div style={{ width: '100%', maxWidth: 360 }}>
          <div style={{ textAlign: 'center', marginBottom: 24 }}>
            ...
            <p style={{ color: '#666', fontSize: '0.78rem', marginTop: 4 }}>Sign in with your admin account</p>
          </div>
          ...
          <form onSubmit={async e => { e.preventDefault(); setLoginErr(''); try { await signIn(email, pass); toast('Signed in successfully!', 'success'); } catch (err) { setLoginErr(err.message); } }}>
            <div style={{ marginBottom: 12 }}>
              ...
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="admin@asiacup.it" required />
            </div>
  ...
  const autoDraw = async () => {
    ...
    try {
      ...
      // 17 teams → Group B takes 5, the rest take 4. Any extra teams spill into B first.
      const base = Math.floor(shuffled.length / letters.length);
      let extra = shuffled.length - base * letters.length;
      const sizes = letters.map(l => l === 'B' && extra > 0 ? (extra--, base + 1) : base);
      letters.forEach((l, i) => { if (l !== 'B' && extra > 0) { sizes[i]++; extra--; } });
      let cursor = 0;
      const gs = Object.fromEntries(letters.map((l, i) => { const slice = shuffled.slice(cursor, cursor + sizes[i]); cursor += sizes[i]; return [l, slice]; }));
    ...
    <div>
      <div style={{ marginBottom: 16 }}>
        ...
        <p style={{ color: '#888', fontSize: '0.78rem', margin: 0 }}>
          View the squad registered by each team's manager. Deadline: 27th September 2026, 9:00 AM.
        </p>
      </div>
