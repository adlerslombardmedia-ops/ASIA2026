...
import { COLORS, TeamLogo, VENUE_NAME, VENUE_MAP_URL, EVENT_DATE_LABEL } from '../lib/hooks';
...
export default function InfoView({ data, navigate }) {
  ...
  return (
    <div className="animate-fade">
      {/* Header */}
      <div className="hero speedlines" style={{ padding: '22px 16px 0' }}>
        <div className="hero-kicker">Tournament</div>
        <h1 className="hero-title" style={{ fontSize: 'clamp(2rem, 9vw, 3rem)' }}><span className="gold-text slant">Info Board</span></h1>
        <div style={{ fontSize: '0.75rem', color: 'var(--text2)', marginTop: 4 }}>Official announcements from the tournament director</div>
        <div className="hero-facts">
          <span className="fact-chip"><strong>{EVENT_DATE_LABEL}</strong></span>
          <a href={VENUE_MAP_URL} target="_blank" rel="noreferrer" className="fact-chip">{VENUE_NAME} · Open in Maps</a>
        </div>
      </div>

      {/* Venue card */}
      <div style={{ padding: '16px 16px 0' }}>
        <a href={VENUE_MAP_URL} target="_blank" rel="noreferrer" className="kcard tappable" style={{ display: 'flex', alignItems: 'center', gap: 14, padding: 14, textDecoration: 'none', color: 'inherit' }}>
          <div style={{ width: 46, height: 46, borderRadius: 10, background: 'var(--gradient-gold-flat)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#0b0b0b" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg></div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: '0.62rem', fontWeight: 800, letterSpacing: 2, color: 'var(--gold)', textTransform: 'uppercase' }}>Venue</div>
            <div style={{ fontWeight: 800, fontSize: '0.95rem' }}>{VENUE_NAME}</div>
            <div style={{ fontSize: '0.74rem', color: 'var(--text2)' }}>Bergamo · {EVENT_DATE_LABEL} · Tap for directions</div>
          </div>
